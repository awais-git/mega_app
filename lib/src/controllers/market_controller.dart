...
class MarketController extends ControllerMVC {
  ...
  //   });
  // }
  Future<void> listenForCategories(String idMarket) async {
    final Stream<Category> stream = await getCategoriesOfMarket(idMarket);
    stream.listen((Category _category) {
      setState(() => categories.add(_category));
    }, onError: (a) {
      print(a);
  ...
  Future<void> refreshMarket( ) async {
    ...
    listenForCategories(_id.toString() );
  }
}
