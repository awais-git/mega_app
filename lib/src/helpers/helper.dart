...
//        },
  ...
  static getTotalEnvio(Market _market) async {
    double _distance =0;
    double extra=0;
    if (_distance == 0 && !deliveryAddress.value.isUnknown()) {
      _distance = await getDistancia(_market,deliveryAddress.value);
      print("ingreso helper");
      print(_distance);
    }
    // getDistancia().then((value)  {
    //   print("ingreso helper");
    //   print(value);
    // });
    if (_distance == 0 && !deliveryAddress.value.isUnknown()) {
    ...
    print(_distance);
    print('deliveryMin--->') ;
    print(setting.value.deliveryMin);
    print('deliveryExtra--->') ;
    print(setting.value.deliveryExtra);
    
    if(_distance > setting.value.deliveryMin){
      extra = (_distance - setting.value.deliveryMin).ceilToDouble();
    }
    print(' Extra===> $extra');
    double envio=_market.deliveryFee+(extra*setting.value.deliveryExtra);
    print(' Envio===> $envio');
    return envio;
  }
  ...
  static bool canDelivery(Market _market, {List<Cart> carts}) {
    ...
    _can &= (_distance < _deliveryRange) && !deliveryAddress.value.isUnknown();
    return _can;
  }
  ...
  String trans(String text) {
    switch (text) {
      ...
      case "App\\Notifications\\CancelOrder":
        return S.of(context).order_status_cancel;
