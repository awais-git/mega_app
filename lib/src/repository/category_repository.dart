...
Future<Stream<Category>> getCategories() async {
  ...
  filter.open = false;
  _queryParams['onlyWithProducts'] = 'true';
...
Future<Stream<Category>> getCategoriesOfMarket(String marketId) async {
  ...
  Map<String, dynamic> _queryParams = {};
  SharedPreferences prefs = await SharedPreferences.getInstance();
  var myJson=json.decode(prefs.getString('filter') ?? '{}');
   myJson['market_id'] = marketId;
   _queryParams['onlyWithProducts'] = 'true';
   
  print(CustomTrace(StackTrace.current, message: "MYJSON: "+ myJson.toString()).toString());
  Filter filter = Filter.fromJSON(myJson);
  filter.delivery = false;
  filter.open = false;
