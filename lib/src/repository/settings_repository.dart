...
Future getDistancia(tienda, cliente) async {
  print("dentro getDistancia");
  final String url = 'https://maps.googleapis.com/maps/api/distancematrix/json?origins=${tienda.latitude},${tienda.longitude}&destinations=${cliente.latitude}%2C${cliente.longitude}&key=${setting.value?.googleMapsKey}';
  print(url);
  try {
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      //print("dentro getDistancia RESPONSE");
      //print(response.body);
      //print("dentro getDistancia RESPONSE 2");
      final parsed = jsonDecode(response.body);
      print("dentro getDistancia RESPONSE 3");
      print(parsed['rows'][0]['elements'][0]['distance']['value']);
      return parsed['rows'][0]['elements'][0]['distance']['value']/1000;
    }else{
      print("dentro getDistancia RESPONSE FALSE");
    }
   } catch (e) {
    print(CustomTrace(StackTrace.current, message: url).toString());
    return Setting.fromJSON({});
...
Future<String> setOrderInformation(String information) async {
  if (information != null) {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('order_information', information);
  }
  return information;
}
Future<String> getOrderInformation() async {
  SharedPreferences prefs = await SharedPreferences.getInstance();
  //await prefs.clear();
  if (prefs.containsKey('order_information')) {
    return prefs.getString('order_information');
  } else {
    return '';
  }
}
